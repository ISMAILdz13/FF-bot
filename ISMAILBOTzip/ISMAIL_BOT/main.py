...
from crypto import (
    encrypt_aes, decrypt_aes, encrypt_packet, decrypt_packet,
    encode_uid, decode_uid, encode_varint, create_variant,
    create_length, create_proto, decode_hex, fix_packet,
    decode_packet, generate_packet, generate_login_packet,
    lag_squad, get_status, spam_room, generate_join_squads_packet,
    generate_join_global, function_fs, send_emote, authenticate_clan,
    authenticate_global, authenticate_chat, send_message,
    send_squad_message, generate_ghost_packet, open_squad,
    check_squad, send_invite, exit_squad, format_message,
    get_user_agent, get_random_color, get_random_banner,
    get_squad_data, send_room_chat_enhanced
)
from helpers import (
    fetch_tokens, get_token, send_emote_request, 
    get_player_likes, request_spam, get_player_name,
    get_player_info, delete_friend, check_uid_status
)
from xC4 import *
...
ADMIN_UID = Config.BOT_IDS['admin_uid']
BOT_UID = Config.BOT_IDS['bot_uid']
BOT_NAME = Config.BOT_NAME
server2 = Config.SERVER_CONFIG['server']
key2 = Config.SERVER_CONFIG['key']
BYPASS_TOKEN = Config.SECURITY['bypass_token']
...
start_spam_duration = Config.TIMING['start_spam_duration']
wait_after_match = Config.TIMING['wait_after_match']
start_spam_delay = Config.TIMING['start_spam_delay']


evo_emotes = {
    "1":  "909000063",   # AK
    "2":  "909000068",   # SCAR
    "3":  "909000075",   # MP40 (1st)
    "4":  "909040010",   # MP40 (2nd)
    "5":  "909000081",   # M1014 (1st)
    "6":  "909039011",   # M1014 (2nd)
    "7":  "909000085",   # XM8
    "8":  "909000090",   # Famas
    "9":  "909000098",   # UMP
    "10": "909035007",   # M1887
...
# Emote mapping for /evo and /evo_fast commands (synced with evo_emotes)
EMOTE_MAP = {
    1:  909000063,   # AK
    2:  909000068,   # SCAR
    3:  909000075,   # MP40 (1st)
...
BADGE_VALUES = {
    ...
    "v1": 1048576,    # V-Badge
    "mod": 2048       # Moderator Badge
}

# Commands Description
COMMANDS_HELP = {
    "/help": "Show this help message",
    "/info <uid>": "Get player basic info",
    "/bio <uid>": "Get player bio",
    "/likes <uid>": "Send 100 likes",
    "/ai <question>": "Ask AI anything",
    "/join <team_code>": "Join a team and perform dual emote",
    "/ghost <team_code>": "Ghost join a team",
    "/lag <team_code>": "Start lag attack on a team",
    "/stop lag": "Stop lag attack",
    "/alltitles <uid>": "Send all titles to a player",
    "/s1-s5 <uid>": "Send specific badge join requests",
    "/spam <uid>": "Spam all badge join requests",
    "/evo <uid> <1-21>": "Send evolution emote",
    "/ig <username>": "Get Instagram profile info",
    "/check <uid>": "Check account ban status"
}
...
def cleanup_cache():
    """Cache cleanup - disabled (undefined variables)"""
    pass  # Variables last_request_time, CLEANUP_INTERVAL, command_cache, MAX_CACHE_SIZE undefined

def get_rate_limited_response(user_id):
    """Rate limiting - disabled (undefined variables)"""
    return True  # Variables last_request_time, RATE_LIMIT_DELAY undefined
...
def load_accounts():
    """Load accounts from accounts.json file in script directory"""
    try:
        ...
        print(f"❌ Accounts file accounts.json not found!")
        return {}
    except Exception as e:
...
class MultiAccountManager:
    def __init__(self):
        # Use path relative to script location instead of current working directory
        self.script_dir = os.path.dirname(os.path.abspath(__file__))
        self.accounts_file = os.path.join(self.script_dir, "accounts.json")
    ...
    async def create_account_join_packet(self, target_uid, account_uid, open_id, access_token, key, iv, region):
        ...
        try:
            ...
            print(f"❌ Error creating account join packet: {e}")
            return None
...
async def SEnd_InV_with_Cosmetics(Nu, Uid, K, V, region):
    ...
    return await generate_packet((await create_proto(fields)).hex(), packet, K, V)   
...
async def RedZed_SendInv(bot_uid, uid, key, iv):
    ...
    try:
        ...
        final_packet = await generate_packet(packet_hex, '0515', key, iv)
...
async def auto_start_loop(team_code, uid, chat_id, chat_type, key, iv, region):
    ...
    while not stop_auto:
        try:
            cycle += 1
            print(f"[AUTO] Cycle #{cycle} — joining team {team_code}")

            # Join team (no chat message — silent, efficient)
            ...
            if cycle % 5 == 0:
                status_msg = f"[B][C][00FFFF]🤖 LW BOT RUNNING\n🎯 Team: {team_code}\n🔄 Cycles done: {cycle}\n⚡ Starts sent: {cycle * spam_count}\n💡 /stop to stop"
            ...
            errors += 1
            print(f"[AUTO] Error in cycle #{cycle}: {e}")
            if errors >= 3:
                error_msg = f"[B][C][FF0000]❌ LW bot stopped after 3 errors.\nLast error: {str(e)[:60]}\nRestart with /lw {team_code}\n"
                await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
                break
    ...
    auto_start_running = False
    stop_auto = False
    print(f"[AUTO] Auto start loop stopped. Cycles completed: {cycle}")
...
async def send_title_msg(chat_id, key, iv):
    ...
    fields = {
        ...
        2: {   # data
            ...
            3: f"{{\"TitleID\":{random.choice([905090075, 904990072, 904990069, 905190079])},\"type\":\"Title\"}}",  # title - use random from available titles
            4: int(datetime.now().timestamp()),  # timestamp
            5: 0,   # chat_type
            6: "en", # language
            9: {    # field9 - player details
                1: "[C][B][FF0000] FLASH_FF",  # Nickname
                2: int(await get_random_avatar()),          # avatar_id
                3: 330,                          # rank
                4: 102000015,                    # badge
    ...
    # Encrypt once, store result, use for both length calculation and concatenation
    encrypted_packet = await nmnmmmmn(packet_hex, key, iv)
    header_length = len(encrypted_packet) // 2
...
async def GeNeRaTeAccEss(uid , password):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=Hr, data=data) as response:
            if response.status != 200:
                print(f"ErroR - GeNeRaTeAccEss HTTP {response.status}")
                return (None, None)
            data = await response.json()
            open_id = data.get("open_id")
            access_token = data.get("access_token")
...
async def SEndMsG(H , message , Uid , chat_id , key , iv):
    ...
    else: raise Exception(f"Unknown chat type: {H} (resolved to {TypE})")
    return msg_packet
...
async def SEndPacKeT(OnLinE , ChaT , TypE , PacKeT):
    ...
    if TypE == 'ChaT':
        if not whisper_writer:
            raise Exception("Chat connection not available (whisper_writer is None)")
        whisper_writer.write(PacKeT)
        await whisper_writer.drain()
    elif TypE == 'OnLine':
        if not online_writer:
            raise Exception("Online connection not available (online_writer is None)")
...
async def ArohiAccepted(uid,code,K,V):
    ...
    return await generate_packet((await create_proto(fields)).hex() , '0515' , K , V)
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        if inPuTMsG.strip() == '/help':
                            help_text = "[B][C][00FF00]═══⚡ ISMAIL BOT COMMANDS ⚡═══\n"
                            for cmd, desc in COMMANDS_HELP.items():
                                help_text += f"[00FFFF]{cmd} [FFFFFF]- {desc}\n"
                            help_text += "[00FF00]═══════════════════════"
...
[FF6347]━━━━━━━━━━━━━━━━━━━━━━━━
[FFFFFF]• rampage  • cannibal  • pinball
[FFFFFF]• devil    • scorpio   • frostfire
[FFFFFF]• paradox  • naruto    • aurora
[FFFFFF]• midnight • itachi    • dreamspace
[FF6347]━━━━━━━━━━━━━━━━━━━━━━━━
[00FF00]Usage: /bundle [name]
[FFFFFF]Example: /bundle midnight
[AAAAAA]Or use ID: /bundle 914000002"""
                                await safe_send_message(response.Data.chat_type, bundle_list, uid, chat_id, key, iv)
                            else:
                                bundle_input = parts[1].lower()
                                # Accept name or direct numeric ID
                                if bundle_input.isdigit():
                                    bundle_id = int(bundle_input)
                                    bundle_name = f"ID:{bundle_id}"
                                else:
                                    bundle_id = BUNDLE.get(bundle_input)
                                    bundle_name = bundle_input

                                if bundle_id is None:
                                    valid_names = " | ".join(k for k in BUNDLE.keys() if k != "pinball")
                                    error_msg = f"[B][C][FF0000]❌ Unknown bundle: '{bundle_input}'\n[FFFFFF]Valid names: {valid_names}\n[AAAAAA]Or use numeric ID directly: /bundle 914000002\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    ...
                                    try:
                                        ...
                                        if bundle_pkt:
                                            ...
                                            success_msg = f"[B][C][00FF00]✅ Bundle sent: {bundle_name} (ID: {bundle_id})"
                                            await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                        else:
...
"""
                        ...
                        # NOTE: must match exactly '/e' word to avoid catching /evo /evos /emot /emote
                        _e_parts = inPuTMsG.strip().split()
                        if _e_parts and _e_parts[0] == '/e':
                        ...
                        if inPuTMsG.strip().startswith('/lw'):
                            ...
                            if len(parts) < 2:
                                ...
                                # Send initial message
                                initial_msg = f"[B][C][00FFFF]🤖 LW BOT STARTED!\n🎯 Team: {team_code}\n⚡ Join→Start→Wait→Leave (repeat)\n⏰ Start spam: {start_spam_duration}s | Wait: {wait_after_match}s\n🔔 Status every 5 cycles\n💡 /stop to stop"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop':
                            print('Processing /stop auto-start command')
                            stop_auto = True
                            auto_start_running = False
                            _stopped_team = auto_start_teamcode or "N/A"
                            auto_start_teamcode = None
                            initial_msg = f"""[B][C][00FFFF]🤖 AUTO START BOT STOPPED !

🎯 Team Code: {_stopped_team}
💡 To restart: /lw [team_code]
"""
                            await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                if not uids or not number or not time_val:
                                    ...
                                    try:
                                        ...
                                        if number_int not in EMOTE_MAP:
                                            ...
                                            error_msg = f"[B][C][FF0000]❌ ERROR! Time must be between 1-100!\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        else:
                                            ...
                                            # SUCCESS MESSAGE
                                            emote_id = EMOTE_MAP[number_int]
                                            success_msg = f"[B][C][00FF00]✅ SUCCESS! Custom evolution emote spam started!\nTargets: {len(uids)} players\nEmote: {number_int} (ID: {emote_id})\nDuration: {time_int} seconds\nInterval: 0.1 seconds\n"
                                            await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                            
                                    except ValueError:
                                        error_msg = f"[B][C][FF0000]❌ ERROR! Invalid format! Use numbers only.\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
...
# IMPROVED TREE-STYLE HELP MENU SYSTEM (Commands in their original menus) 🌳
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # Header
                            header = f"[b][c]{get_random_color()}Hey User Welcome To ISMAIL FF's BOT"
...
[FF1493]║ ⚡ [B][FFFF00]BOT INFO[FFFF00][/B] ⚡
[00FFFA]║
[FFFF00]║ 👤 Developer    :: [FF1493] ISMAIL FF 
[32CD32]║ 💻 Status        :: [32CD32]ONLINE
[1E90FF]║ 🛠 Version      :: [1E90FF]ENHANCED V2
[00FFFA]╚═•══•════════════════•══•═╝"""
...
async def MaiiiinE():
    Uid , Pw = '4570917992','E4A3CF30DAE6E197DA90BF89BDBCBDA7FAC4966D6A3C4CBB5E7ECC3A0E153C3A'
    

    open_id , access_token = await GeNeRaTeAccEss(Uid , Pw)
    if not open_id or not access_token:
        print("ErroR - InvaLid AccounT — waiting 30s before retry...")
    ...
    if not MajoRLoGinResPonsE:
        print("TarGeT AccounT => BannEd / NoT ReGisTeReD ! — waiting 60s...")
    ...
    try:
        if ":" in OnLinePorTs:
            OnLineiP, OnLineporT = OnLinePorTs.split(":", 1)
        else:
            print(f"⚠️ Invalid OnLinePorTs format: {OnLinePorTs}")
            return None

        if ":" in ChaTPorTs:
            ChaTiP, ChaTporT = ChaTPorTs.split(":", 1)
        else:
            print(f"⚠️ Invalid ChaTPorTs format: {ChaTPorTs}")
            return None
    except ValueError as e:
    ...
    send_emote_request(ToKen,UrL)  # Send emote choice request to server
    ...
    os.system('clear')
    print("Initializing ISMAIL FF Bot...")
    print("┌────────────────────────────────────┐")
    print("│ █████████████░░░░░░░░░░░░░░░░░░ │")
    print("└────────────────────────────────────┘")
    time.sleep(0.5)
...
def handle_keyboard_interrupt(signum, frame):
    """Clean handling for Ctrl+C and SIGTERM"""
    print(f"\n\n🛑 Bot signal {signum} received — will restart via wrapper...")

# Register signal handlers — do NOT sys.exit so the shell wrapper can restart
...
async def StarTinG():
    ...
    while True:
        ...
        try:
            ...
            if elapsed < 30:
                print(f"Bot exited quickly ({elapsed:.1f}s) — waiting {retry_delay}s before retry...")
                await asyncio.sleep(retry_delay)
            else:
                await asyncio.sleep(3)
        except KeyboardInterrupt:
            print("\n\n🛑 Bot shutdown by user")
            print("👋 Thanks for using ISMAIL FF!")
            break
        except asyncio.TimeoutError:
            ...
            elapsed = asyncio.get_event_loop().time() - start_time
            print(f"ErroR TcP - {e} => ResTarTinG in {retry_delay}s ...")
            await asyncio.sleep(retry_delay)
